...
class DatePickerView: UIView, NibLoadable, UIGestureRecognizerDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    class func datePicker(frame:CGRect, style: DateStyle = .all, scrollToDate: Date = Date(), completionHandler: @escaping (_ date: Date?) -> Void) -> DatePickerView {
    ...
    func show() {
        ...
        UIView.animate(withDuration: 0.3, animations: {
            ...
            self.sureButtonBackgroundColor = UIColor.orange
...
extension DatePickerView {
    ...
    private func currentDate() -> Date? {
        ...
        case .hourMinute:
            ...
            return Date.date(dateStr, formatter: "HH:mm")
        }
...
extension DatePickerView {
    private func scrollTo(_ date: Date) {
        ...
        case .hourMinute:
            hourMinuteStyleScrollTo(scrollToDate)
        }
    ...
    private func hourMinuteStyleScrollTo(_ date: Date) {
...
extension DatePickerView {
    ...
    private func initDataWithHourMinuteStyle() {
        
        if let date = Date.date("00:00", formatter: "HH:mm") {
            minLimitDate = date
        }
        
        if let date = Date.date("23:59", formatter: "HH:mm") {
            maxLimitDate = date
        }
